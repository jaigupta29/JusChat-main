...
import com.example.juschat.adapters.ChatAdapter
import com.example.juschat.models.Inbox
import com.example.juschat.models.User
import com.example.juschat.utils.EncryptMessages
import com.example.juschat.utils.KeyboardVisibilityUtil
import com.example.juschat.utils.isSameDayAs
        ...
        sendBtn.setOnClickListener {
            FirebaseFirestore.getInstance().collection("users").document(friendId).get()
                .addOnSuccessListener {
                    ...
                    if(friendUser.deviceToken!=currentUser.deviceToken){
                        Toast.makeText(this,"set encryption to ${friendUser.deviceToken}",Toast.LENGTH_SHORT).show()
                    }
                    else{
                        var s = EncryptMessages.encrypt(msgEdtv.text.toString())
                        msgEdtv.text?.let {
                            if (it.isNotEmpty()) {
                                sendMessage(s)
                                it.clear()
